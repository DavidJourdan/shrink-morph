...
def run():
    ...
    parser.add_argument(
        "printer_profile", help="The printer profile.")
    parser.add_argument(
        "-o",
        "--output",
    ...
    object_width_xy_plus_margin = object_width_xy + 15

    # Get the printer profile header and footer
    source_folder = os.path.dirname(__file__)
    printer_profile_path_head = f"{source_folder}/printers/{printer_profile}/"
    ...
    if not origin_is_bed_center:
        ...
        for i in range(ncols):
            for j in range(nrows):
                centerX = bed_size_xy[0]/ 2 + (i - (ncols-1) / 2) * object_width_xy_plus_margin[0]
                centerY = bed_size_xy[1]/ 2 + (j - (nrows-1) / 2) * object_width_xy_plus_margin[1]
                for trajectory in trajectories:
                    trajectories2.append(trajectory + np.array([centerX, centerY, 0]))
    ...
    for i in range(5):
        rect_trajectory = []
        X = ncols * object_width_xy_plus_margin[0] / 2 - printer_param.nozzle_width * i
        Y = nrows * object_width_xy_plus_margin[1] / 2 - printer_param.nozzle_width * i
        rect_trajectory.append([bed_size_xy[0]/ 2 - X, bed_size_xy[1]/ 2 - Y, printer_param.layer_height])
        rect_trajectory.append([bed_size_xy[0]/ 2 + X, bed_size_xy[1]/ 2 - Y, printer_param.layer_height])
        rect_trajectory.append([bed_size_xy[0]/ 2 + X, bed_size_xy[1]/ 2 + Y, printer_param.layer_height])
        rect_trajectory.append([bed_size_xy[0]/ 2 - X, bed_size_xy[1]/ 2 + Y, printer_param.layer_height])
        rect_trajectory.append([bed_size_xy[0]/ 2 - X, bed_size_xy[1]/ 2 - Y, printer_param.layer_height])
        rect_trajectories.append(np.array(rect_trajectory))
    trajectories = rect_trajectories + trajectories2
...
M205 S0 T0 ; sets the minimum extruding and travel feed rate, mm/sec
    ...
    # print(f"Total length = {printer_param.total_extrusion_length / 1000:.2f}m")
    # print(f"Weight estimation = {printer_param.total_extrusion_length * 1.25 / 1000:.2f}g")
    # t = total_dist / printer_param.print_speed / 60
    # print(f"Time estimation = {t // 60:.0f} hours {t % 60:.0f} minutes")
